...
const onConnected = () => {
    ...
    stompClient.subscribe('/topic/users', onUserConnected, {})
...
const sendMessage = (event) => {
    const receiverIdElement = document.querySelector('#user-dropdown')
    const receiverUserId = receiverIdElement.value.trim()
    const destination = receiverUserId ? "/app/chat.private.send." + receiverUserId : "/app/chat.send"
...
const onUserConnected = (payload) => {
    var users = Object.entries(JSON.parse(payload.body));
    users.sort((a, b) => a[0].toLowerCase() > b[0].toLowerCase() ? 1 : -1);
